            ...
            return (
              <div key={d.label} className="flex flex-col items-center gap-1.5 flex-none w-9">
                <button
                  ...
                  className={`w-full flex flex-col justify-center items-center gap-0.5 px-1 py-1 rounded transition-all touch-none select-none ${isHidden ? 'opacity-100' : isOff ? 'opacity-30' : 'opacity-100'}`}
                ...
                >
                  {isHidden ? (
                    <>
                      ...
                      <span className="text-[10px] font-mono text-destructive line-through text-center leading-none">{d.label}</span>
                    </>
                  ) : (
                    <>
                      <div className="w-4 h-4 rounded-full" style={{ backgroundColor: `hsl(${d.color})` }} />
                      <span className="text-[10px] font-mono text-muted-foreground text-center leading-none">{d.label}</span>
                    </>
                  )}
                </button>
                <button
          ...
          {stringOrder.map((stringIdx, row) => {
            ...
            return (
              <div key={stringIdx} className="flex items-center relative" style={{ height: stringH }}>
                ...
                {/* String line */}
                {!isDisabled && (
                  <div className="absolute bg-fretboard-string z-[5]" style={{ height: thickness, top: '50%', transform: 'translateY(-50%)', opacity: mutedDim ? 0.25 : 0.9, left: 28, right: 0 }} />
                )}
                ...
                <div className="flex items-center flex-1">
                  {frets.map(fret => {
                    ...
                    return (
                      <div key={fret} className="flex items-center justify-center relative" style={{ width: `${widths[fret]}%`, height: stringH }}>
                    {fret > 0 && <div className="absolute left-0 top-0 bottom-0 bg-fretboard-fret" style={{ width: 2, opacity: 0.55 }} />}
