...
export function ScaleRootSelector({ selectedRoot, onSelect }: { selectedRoot: NoteName; onSelect: (n: NoteName, display?: string) => void }) {
  ...
  return (
    <div className="mb-2">
      <div className="flex flex-nowrap gap-0.5 items-end">
        {NATURAL_NOTES.map(n => {
                ...
                <div className="mt-0.5 flex gap-px">
                  {showFlat && (
                    <button
                      ...
                      className={`w-4 h-4 ${showSharp ? 'rounded-l' : 'rounded'} border text-[9px] font-mono font-bold transition-colors ${
                        accidental === 'flat'
                  ...
                  {showSharp && (
                    <button
                      ...
                      className={`w-4 h-4 ${showFlat ? 'rounded-r border-l-0' : 'rounded'} border text-[9px] font-mono font-bold transition-colors ${
                        accidental === 'sharp'
                          ? 'bg-primary text-primary-foreground border-primary'
                          : 'bg-muted/40 border-border/60 text-muted-foreground hover:bg-muted'
                      }`}
